...
//  bad NODE located
      ...
      if (arRow>0 && (arCol+1)<=maxCol) {
        avgFTemp[arRow-1][arCol+1] = '|';
      }
      console.log('row+1 '+(arRow+1)+' max '+maxRow);
      if ((arRow+1)<=maxRow) {
        avgFTemp[arRow+1][arCol] = '|';
      }
      console.log('row '+arRow+' col+1 '+(arCol+1)+' max '+maxCol);
      if ((arRow+1)<=maxRow && (arCol+1)<=maxCol) {
        avgFTemp[arRow+1][arCol+1] = '|';
        }
      if ((arRow+1)<=maxRow && arCol>0) {
        avgFTemp[arRow+1][arCol-1] = '|';
       }
      if ((arCol+1)<maxCol) {
        avgFTemp[arRow][arCol+1] = '|';
